...
- Any caller with Case work access may append Records while the Case is open, and may assign, retitle, close, reopen, and connect handoffs. Only the Record's creator may update or redact it; system Records cannot be changed. Access changes and archiving stay with the current Case assignee; `started_by` is history. Editors can create and update Tasks. Review Task resolution stays with the named reviewer.
