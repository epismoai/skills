...
- **Case access:** Case access never inherits from its Playbook. It may include `public`, which is read-only and exposes the current title, input, Records, and readable handoff neighborhood; it never exposes Tasks, assignment, or collaborator identities. Tasks and Records have no access list of their own and follow the current parent Case. Anyone with Case work access may append Records to an open Case; only the creator may update or redact their own non-system Record. Managing access and archiving remain limited to the current assignee, while editors can create Tasks, retitle, request an Epismo AI review, and change whether OUTPUT Records enqueue one.
