...
Use this guide for access, aliases, share tokens, public visibility, and archival. A case and a playbook have independent access; pick the mechanism that matches the object.
...
- **Share URL:** an opaque token URL for a case or a playbook. The current web flow still applies the target's live access check; the token itself does not widen access.
